        ...
        <div>
            <div className="flex flex-col  p-6 space-y-6 overflow-hidden rounded-lg shadow-lg bg-gray-50 text-black ">
                ...
                <div>
                    ...
                    {
                        details.length > 150 ?
                            <p className="text-sm text-gray-800">{details.slice(0, 150) + '...'}<Link to={`/courses/details/${_id}`}><span className='text-blue-600'>read more</span></Link></p>
                            :
                            <p>{details}</p>
                    }
                ...
                <Link to={`/courses/details/${_id}`} type="button" className="flex items-center justify-center w-3/4 mx-auto p-3 font-semibold tracking-wide rounded-md btn-info bg-gradient-to-r from-cyan-600 to-info ">See Details</Link>
            </div>
        </div >
    );
};
